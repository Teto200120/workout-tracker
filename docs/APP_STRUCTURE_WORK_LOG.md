...
- Feedback diagnostic validation correction: the same safe receiver failure-code allowlist is accepted by the existing feedback-report validator before the outbox is written, so a failed delivery remains safely visible with its diagnostic state. The service-worker cache was bumped to `hector-workout-tracker-pwa-v41`.
- Feedback client-state diagnosis: retry failures now distinguish unavailable verification from a fetch-level network failure, and the Worker emits a live-tail-only persistence stage label if its safe cross-store write fails. Neither signal includes report text, IDs, IPs, screenshot data, secrets, or persistent telemetry. The service-worker cache was bumped to `hector-workout-tracker-pwa-v42`.
- Feedback delivery root-cause correction: the live transport now sends the already stripped payload supplied by the feedback service instead of validating it a second time. This restores the actual network request while preserving the unchanged queue-before-send and outbox safety behavior. The service-worker cache was bumped to `hector-workout-tracker-pwa-v43`.
...
- Bump that pre-1.0 semantic version with each production release (patch for fixes, minor for user-visible features) and bump the matching service-worker cache name in the same change. Reserve `1.0.0` for an explicit stability decision.
