...
- The ordered schema 0-to-1-to-2 migration, display-name defaulting/normalization, deterministic/idempotent behavior, invalid input, and current-version validation.
- Display-name Unicode boundaries, invisible/control rejection, onboarding-required decisions, non-mutating settings preservation, single-flight save failure, and retry.
- Provider-independent Active Workout/Routine picker sessions plus non-mutating routine add/replace and duplicate-index rules.
...
- First-launch no-Home-flash gating, international/markup-like display names, inline validation, failed-write retry with retained input, rapid submission, later edit/cancel/failure, immediate Home/Profile refresh, and reload.
...
- Routine builder use of the same picker for local/catalog/custom add, replace, cancel/focus return, duplicate confirmation, single selection, unsaved-draft isolation, explicit save, reload, and catalog-failure fallback.
...
- Backup download with display name, clear-to-onboarding, restore without legacy `weights`, malformed-name rejection, invalid import rejection, and failed-transaction rollback.
- Legacy startup migration across IndexedDB and localStorage into schema 2 plus onboarding, current startup without rewrite or duplication, failed migration preservation/retry, and future application-version refusal.
- Backup version declarations, old v2 name-less migration and onboarding, new v3/schema-2 name round trip, future-version refusal, clear-data marker removal, and retained `id: null` rollback.
...
On the Samsung Galaxy S24 Ultra, repeat first-run onboarding and name editing; Home, Active Workout, Routine Browse/Change, and Exercise Details scroll/action/keyboard checks; collapsed filter/count/reset and short local/catalog hierarchy; preview-versus-full-Guide behavior; exact/alias/custom/history name resolution; draft reload; save/edit; legacy startup migration; failed-migration recovery; backup clear/restore; offline routine/catalog/Guide reload; haptic; and portrait-layout checks from the release checklist.
