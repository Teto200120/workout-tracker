...
Exercise-catalog API integration remains deferred. Schema 1 deliberately adds no API ID, provider, remote image, equipment taxonomy, muscle taxonomy, sync status, catalog/custom discriminator, account ownership, or Android-only field. A later catalog task must define identity and compatibility rules as a separate schema migration. This document is the platform-neutral data blueprint for that work and for an eventual Android implementation; it is not an Android storage migration.
